...
@app.route("/api/fixture-packages/export-pdf", methods=["POST"])
@login_required
def api_export_fixture_pdf():
    try:
        from pypdf import PdfWriter, PdfReader
    except ImportError:
        return jsonify({"ok": False, "error": "pypdf not installed on server"}), 503

    table_pdf_file = request.files.get("table_pdf")
    if not table_pdf_file:
        return jsonify({"ok": False, "error": "Missing table PDF"}), 400

    catalog_ids = json.loads(request.form.get("catalog_ids", "[]"))
    row_refs    = json.loads(request.form.get("row_refs",    "[]"))

    writer = PdfWriter()

    # Add table pages generated by the client
    table_reader = PdfReader(io.BytesIO(table_pdf_file.read()))
    for page in table_reader.pages:
        writer.add_page(page)

    # Collect spec R2 keys from catalog rows and row-attachment rows
    spec_files = []
    for cid in catalog_ids:
        try:
            for s in get_fixture_specs(int(cid)):
                spec_files.append((s["r2_key"], s["file_type"]))
        except Exception:
            pass
    for ref in row_refs:
        try:
            for a in get_row_attachments(ref.get("estimate_name", ""), ref.get("row_id", "")):
                spec_files.append((a["r2_key"], a["file_type"]))
        except Exception:
            pass

    # Append spec pages (images converted to PDF, PDFs appended directly)
    if spec_files and r2_utils.ENABLED:
        for r2_key, file_type in spec_files:
            try:
                file_bytes = r2_utils.get_file(r2_key)
                if file_type == "application/pdf":
                    spec_reader = PdfReader(io.BytesIO(file_bytes))
                    ...
                    img_buf.seek(0)
                    for page in PdfReader(img_buf).pages:
                        writer.add_page(page)
            except Exception:
                pass  # Skip any spec that fails; don't abort the whole export

    output = io.BytesIO()
    writer.write(output)
    output.seek(0)
    return send_file(output, mimetype="application/pdf", as_attachment=True,
                     download_name="fixture-schedule.pdf")
